	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("user") == null) {
			...
			if (user != null) {// 查找成功
				...
				List<DegreeRecord> degreeRecords=degreeRecordService.queryCoachDegreeByUserId(user.getUserId());
				if(degreeRecords.size()!=0){
					map.put("hasDegree", "yes");
					map.put("degreeRecords", degreeRecords);
					session.setAttribute("degreeRecords", degreeRecords);
				}else{
		...
		if ("0".equals(map.get("result"))) {// 登录成功，且不是第一次登陆
			writer.println("<script language='javascript'>window.location.href='./views/coach/degree.jsp'</script>");
