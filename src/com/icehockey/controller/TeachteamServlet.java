	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("user") == null) {
			...
			if (user != null) {// 查找成功
				...
				List<TeamRecord> teamRecords=teamRecordService.queryTeamRecordByUserId(user.getUserId());
				if(teamRecords!=null){
					map.put("teamRecords", teamRecords);
					session.setAttribute("teamRecords", teamRecords);
				}else{
					System.out.println("未找到执教俱乐部记录");
				}
				List<ClubRecord> clubRecords=ClubRecordService.queryClubRecordByUserId(user.getUserId());
				if(clubRecords!=null){
					map.put("clubRecords", clubRecords);
					session.setAttribute("clubRecords", clubRecords);
				}else{
					System.out.println("未找到执教俱乐部记录");
				}
		...
		if ("0".equals(map.get("result"))) {// 登录成功，且不是第一次登陆
			writer.println("<script language='javascript'>window.location.href='./views/coach/teamAndclub.jsp'</script>");
